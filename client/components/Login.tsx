...
const Login = () => {
	...
	return <div className='shadow-2xl rounded-lg flex flex-col p-5'>
		...
		<div className='flex justify-between mx-4 my-2'>
			<button id='sign-in-button' className='mr-2 rounded-lg font-semibold py-1 px-20 transition-all hover:shadow-2xl hover:scale-110' onClick={() => context?.signIn(name, password)}>Sign In</button>
			<button id='sign-up-button' className='ml-2 rounded-lg font-semibold py-1 px-20 transition-all hover:shadow-2xl hover:scale-110' onClick={() => context?.signUp(name, password)}>Sign Up</button>
		</div>
	</div>
}
